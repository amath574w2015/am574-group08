...
\par
The arrival time of the peak predicted by geoclaw is about 0.08 second earlier than the time measured from experiment and in experiment, the jump is more like a smooth increase. 
If we zoom in figure \ref{fig:waveheight_nocolumn} for smaller time range from 3s to 4s, we get \ref{fig:waveheight_nocolumn_zoomin}.
In \ref{fig:waveheight_nocolumn_zoomin}, arrival time of peak predicted by geoclaw is about 0.11 second earlier that that from experiment.  
...
This causes the shock remain its shape in as it propagates in shallow water equation while smooth out in experiment.
...
In other words, it even does not know there is a column before the shock hits the column. In contrast, N-S equation is a elliptic PDE. 
...
\begin{figure}[h!]
    ...
    \caption{time history of wave height for case without column}
