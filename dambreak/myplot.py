...
def plot_no_column():
    ...
    gaugeno = 7#this is actually gauge at center of column
    g1 = plotdata.getgauge(gaugeno)
    gaugeno = 6#this is actually gauge at farther away from column
    ...
    plt.savefig('./myplot/waveheight_nocolumn_zoomin.png', bbox_inches='tight')
    plt.close()
...
def plot_cylinder():
    ...
    gaugeno = 2                   # gauge number to examine
    g_gauge2 = plotdata.getgauge(gaugeno)
    ...
    with open('./dataForComp/waveheight_cylinder_dist=0_largerTimeRange.csv', 'rb') as csvfile:
        reader = csv.reader(csvfile)
        wave_height_cylinder_gauge2 = list(reader)
        wave_height_cylinder_gauge2 = np.array(wave_height_cylinder_gauge2)
        wave_height_cylinder_gauge2 = wave_height_cylinder_gauge2.astype(np.float)#convert string to floats

    #plot waveheight history
    plt.figure()
    #plt.plot(g1.t,g1.q[0,:],'b-',label='geoclaw')
    #plt.plot(g2.t,g2.q[0,:],'g-',label='geoclaw2')
    plt.plot(g_gauge2.t,g_gauge2.q[0,:],'b-',label='geoclaw')
    plt.plot(wave_height_cylinder_gauge2[:,0],wave_height_cylinder_gauge2[:,1],'r--',label='Exp')
    legend = plt.legend(loc='lower center', shadow=True, fontsize='x-large')
    plt.xlabel('time (s)')
    plt.ylabel('wave height (m)')
    plt.title('wave height history at x=11.04')
    plt.xlim(3.0,10.0)
    #plt.xlim(3.0,10.0)
    plt.savefig('./myplot/waveheight_cylinder_x1104_largerTimeScale.png', bbox_inches='tight')
    plt.close()
...
def plot_square():
    ...
    plotdata = ClawPlotData()
    plotdata.outdir = '_output_square_4'   # set to the proper output directory
    gaugeno = 2                   
    g_gauge2 = plotdata.getgauge(gaugeno)
    #gaugeno = 4                   
    #g_gauge4 = plotdata.getgauge(gaugeno)

    #get data from case without column
    #plotdata = ClawPlotData()
    #plotdata.outdir = '_output_nocolumn_3'   # set to the proper output directory
    #gaugeno = 6                   # gauge number to examine
    #g_nocolumn = plotdata.getgauge(gaugeno)
    ...
    #with open('./dataForComp/waveheight_square_dist=0_largerTimeRange.csv', 'rb') as csvfile:
    with open('./dataForComp/waveheight_square_dist=0.csv', 'rb') as csvfile:
        reader = csv.reader(csvfile)
        wave_height_square_gauge2 = list(reader)
        wave_height_square_gauge2 = np.array(wave_height_square_gauge2)
        wave_height_square_gauge2 = wave_height_square_gauge2.astype(np.float)#convert string to floats
        wave_height_square_gauge2[:,1] = wave_height_square_gauge2[:,1]/1000.0

    #with open('./dataForComp/waveheight_square_dist=-20.csv', 'rb') as csvfile:
    #    reader = csv.reader(csvfile)
    #    wave_height_square_gauge4 = list(reader)
    #    wave_height_square_gauge4 = np.array(wave_height_square_gauge4)
    #    wave_height_square_gauge4 = wave_height_square_gauge4.astype(np.float)#convert string to floats
    #with open('./dataForComp/waveheight_square_dist=120.csv', 'rb') as csvfile:
    #    reader = csv.reader(csvfile)
    #    wave_height_square_gauge1 = list(reader)
    #    wave_height_square_gauge1 = np.array(wave_height_square_gauge1)
    #    wave_height_square_gauge1 = wave_height_square_gauge1.astype(np.float)#convert string to floats
    #    wave_height_square_gauge1[:,1] = wave_height_square_gauge1[:,1]/1000.0

    #plot waveheight history
    plt.figure()
    #plt.plot(g1.t,g1.q[0,:],'y-',label='geoclaw1')
    #plt.plot(g2.t,g2.q[0,:],'g-.',label='geoclaw2')
    #plt.plot(g3.t,g3.q[0,:],'b-',label='geoclaw_square')
    #plt.plot(wave_height_square[:,0],wave_height_square[:,1],'r--',label='Exp')
    plt.plot(g_gauge2.t,g_gauge2.q[0,:],'b-',label='geoclaw')
    #plt.plot(g_nocolumn.t,g_nocolumn.q[0,:],'r--',label='geoclaw_nocolumn')
    plt.plot(wave_height_square_gauge2[:,0],wave_height_square_gauge2[:,1],'r--',label='Exp')
    legend = plt.legend(loc='upper center', shadow=True, fontsize='x-large')
    plt.xlabel('time (s)')
    plt.ylabel('wave height (m)')
    plt.title('wave height history at x=11.04')
    #plt.title('wave height history at x=11.0 for two different cases')
    #plt.title('wave height history at x=11.0')
    plt.xlim(3.0,4.0)
    #plt.xlim(3.0,10.0)
    #plt.savefig('./myplot/waveheight_x11_comparison.png', bbox_inches='tight')
    plt.savefig('./myplot/waveheight_square_x1104.png', bbox_inches='tight')
    plt.close()
